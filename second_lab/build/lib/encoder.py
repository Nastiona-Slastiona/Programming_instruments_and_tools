...
def function_to_obj(obj):
    fglobals = {key: value for key, value in _copy_func(obj)['globals'].items() if key in _copy_code(obj)['names']}
    _copy_func(obj).update({'globals': fglobals})
    func_dict = {'CodeType': _copy_code(obj), 'FunctionType': _copy_func(obj)} 
    return func_dict
